...
% Modified by Ren Zhang 7/3/2018
if ~exist('Vars2Show','var'),Vars2Show = ObsVar;end
%if ~exist('Vars2Show','var'),Vars2Show = StateVar;end
% Vars2Show0=Vars2Show;
% Vars2Show{1,1}=[Vars2Show{1,1},'rnat'];
% Vars2ShowPretty = Vars2Show;
...
if nDrawsUsed>1
   % fprintf(' (Percentile: %.1f)',VDPrctile)  Ren Zhang 
   fprintf(' (Mean (selected by Ren Zhang)')
end
fprintf('\n')
for jPanel=1:nPanels
    ...
    for jh=xTickIdx
        if nDrawsUsed==1
          ...
          %  VDj = prctile(ShowVDj(:,:,jh,:),VDPrctile,4); by Ren Zhang
          VDj = mean(ShowVDj(:,:,jh,:),4);
        end
        fprintf('\nHorizon: %.0f\n',VDHorizons(jh))
...
for ii=1:nPanels
    ...
    for jj=1:length(Vars2Show{ii})
        ...
        if nDrawsUsed==1
           ...
           % PlotData = squeeze(prctile(ShowVDj(jj,:,:,:),VDPrctile,4)); by
           PlotData = squeeze(mean(ShowVDj(jj,:,:,:),4));
        end
...
% Add by Ren Zhang, 7/2/2018

% clear Vars2ShowPretty;
% clear Vars2Show;

% Vars2Show=Vars2show0;
% Vars2ShowPretty=Vars2show;
