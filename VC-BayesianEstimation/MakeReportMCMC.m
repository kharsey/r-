...
% Updated: July 4, 2018 by Ren Zhang to replace .pdf with .eps
% 
...
% % The last two statements are added by Ren Zhang to output historical decomp and forecast, 7/3/2018
% if ~exist('isMakePlotsHistDecomp','var'),isMakePlotsHistDecomp=1;end
% if ~exist('isMakePlotsForecast','var'),isMakePlotsForecast=1;end
...
if any(ismember({'All','PriorPost'},ReportSections))
  ...
  for jF=1:ceil(np/(FigShape{1}*FigShape{2}))
    ...
    fprintf(fid,'\\includegraphics[scale=1]{%s%sFig%.0f.eps}\n',...
...
if any(ismember({'All','States'},ReportSections))&&isMakePlotsStates
  ...
  for jF=1:ceil(nStateVar/(FigShapeStates{1}*FigShapeStates{2}))
    ...
    fprintf(fid,'\\includegraphics[scale=1]{%s%sFig%s.eps}\n',...
    ...
    for jPanel=1:nPanels
        for jShock=1:nShocks2Show
            ...
            fprintf(fid,'\\label{IRF_%s_%s}\n',...
                    Panels{jPanel},Shocks2Show{jShock});
            fprintf(fid,'\\includegraphics[scale=1]{%s%s_%s_%s.eps}\n',...
...
if any(ismember({'All','VD'},ReportSections))&&isMakeVD
    ...
    for jPanel=1:nPanels
        ...
        fprintf(fid,'\\includegraphics[scale=1]{%s%s_%s.eps}\n',...
